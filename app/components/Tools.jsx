...
export default function ToolsSection({ openModal }) {
  return (
    <section
      id="tools-section"
      className="relative w-full bg-conti-blue text-white pb-16"
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full max-w-7xl mx-4 px-4">
          {tools.map((tool, index) => (
            <Link href={tool.link} key={index} className="no-underline">
              <div className="p-8 rounded-lg flex flex-col items-center text-center hover:bg-sky-600 transition-all duration-300 ease-in-out">
                {tool.icon}
                <h3
